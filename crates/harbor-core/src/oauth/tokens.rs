...
pub fn refresh_access_token(req: TokenRefreshRequest<'_>) -> Result<OAuthTokenSet> {
    tracing::info!("Refreshing OAuth access token at {}", req.token_url);
    ...
    if !status.is_success() {
        tracing::warn!("OAuth token refresh failed with HTTP {status}");
    ...
    tracing::info!("OAuth access token refreshed successfully");
    Ok(tokens)
}
...
pub(crate) fn exchange_code(
    ...
    tracing::info!("Exchanging authorization code at {token_url}");
    let client = reqwest::blocking::Client::new();
    let mut form = vec![
    ...
    let body = response.text()?;
    if !status.is_success() {
        tracing::warn!("OAuth authorization code exchange failed with HTTP {status}");
